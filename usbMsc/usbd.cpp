...
void USBD_GetDescriptor (USBD_HandleTypeDef* usbdHandle, USBD_SetupReqTypedef* req) {
  ...
  switch (req->wValue >> 8) {
    ...
    case USB_DESC_TYPE_BOS:
      buf = usbdHandle->pDesc->GetBOSDescriptor (usbdHandle->dev_speed, &len);
      break;
    //}}}
    #endif
    //{{{
    case USB_DESC_TYPE_DEVICE:
      buf = usbdHandle->pDesc->GetDeviceDescriptor(usbdHandle->dev_speed, &len);
      break;
    //}}}
    //{{{
    case USB_DESC_TYPE_CONFIGURATION:
      if(usbdHandle->dev_speed == USBD_SPEED_HIGH ) {
        buf   = (uint8_t *)usbdHandle->pClass->GetHSConfigDescriptor(&len);
        buf[1] = USB_DESC_TYPE_CONFIGURATION;
        }
      else {
        buf   = (uint8_t *)usbdHandle->pClass->GetFSConfigDescriptor(&len);
        buf[1] = USB_DESC_TYPE_CONFIGURATION;
        }
      break;
    //}}}
    ...
    case USB_DESC_TYPE_STRING:
      switch ((uint8_t)(req->wValue)) {
        ...
        case USBD_IDX_LANGID_STR:
          buf = usbdHandle->pDesc->GetLangIDStrDescriptor(usbdHandle->dev_speed, &len);
          break;
        //}}}
        //{{{
        case USBD_IDX_MFC_STR:
          buf = usbdHandle->pDesc->GetManufacturerStrDescriptor(usbdHandle->dev_speed, &len);
          break;
        //}}}
        //{{{
        case USBD_IDX_PRODUCT_STR:
          buf = usbdHandle->pDesc->GetProductStrDescriptor(usbdHandle->dev_speed, &len);
          break;
        //}}}
        //{{{
        case USBD_IDX_SERIAL_STR:
          buf = usbdHandle->pDesc->GetSerialStrDescriptor(usbdHandle->dev_speed, &len);
          break;
        //}}}
        //{{{
        case USBD_IDX_CONFIG_STR:
          buf = usbdHandle->pDesc->GetConfigurationStrDescriptor(usbdHandle->dev_speed, &len);
          break;
        //}}}
        //{{{
        case USBD_IDX_INTERFACE_STR:
          buf = usbdHandle->pDesc->GetInterfaceStrDescriptor(usbdHandle->dev_speed, &len);
          break;
        //}}}
        //{{{
        default:
        #if (USBD_SUPPORT_USER_STRING == 1)
          buf = usbdHandle->pClass->GetUsrStrDescriptor(usbdHandle, (req->wValue) , &len);
          break;
        #else
          USBD_CtlError(usbdHandle , req);
          return;
    ...
    case USB_DESC_TYPE_DEVICE_QUALIFIER:
      if (usbdHandle->dev_speed == USBD_SPEED_HIGH  ) {
        buf = (uint8_t *)usbdHandle->pClass->GetDeviceQualifierDescriptor(&len);
        break;
        }
      else {
        USBD_CtlError (usbdHandle , req);
    ...
    case USB_DESC_TYPE_OTHER_SPEED_CONFIGURATION:
      if (usbdHandle->dev_speed == USBD_SPEED_HIGH  ) {
        buf   = (uint8_t *)usbdHandle->pClass->GetOtherSpeedConfigDescriptor(&len);
  ...
  if ((len != 0)&& (req->wLength != 0)) {
    ...
    USBD_CtlSendData (usbdHandle, buf, len);
    }
  }
//}}}
//{{{
void USBD_SetAddress (USBD_HandleTypeDef* usbdHandle, USBD_SetupReqTypedef* req) {
  ...
  if ((req->wIndex == 0) && (req->wLength == 0)) {
    ...
    if (usbdHandle->dev_state == USBD_STATE_CONFIGURED)
      ...
      usbdSetUSBAddress (usbdHandle, dev_addr);
      USBD_CtlSendStatus (usbdHandle);
      if (dev_addr != 0)
        usbdHandle->dev_state  = USBD_STATE_ADDRESSED;
      else
  ...
  switch (req->bRequest) {
      ...
      switch (usbdHandle->dev_state) {
        ...
        case USBD_STATE_ADDRESSED:
          if ((ep_addr != 0x00) && (ep_addr != 0x80))
            usbdStallEP (usbdHandle , ep_addr);
          break;
        //}}}
        //{{{
        case USBD_STATE_CONFIGURED:
        ...
        case USBD_STATE_CONFIGURED:
          if (req->wValue == USB_FEATURE_EP_HALT) {
            if ((ep_addr & 0x7F) != 0x00) {
              usbdClearStallEP (usbdHandle , ep_addr);
              usbdHandle->pClass->Setup (usbdHandle, req);
            }
            USBD_CtlSendStatus (usbdHandle);
          }
    ...
    case USB_REQ_GET_STATUS:
      ...
      case USBD_STATE_CONFIGURED:
        ...
        if(usbdIsStallEP (usbdHandle, ep_addr))
          pep->status = 0x0001;
        else
          pep->status = 0x0000;
...
void USBD_CtlError (USBD_HandleTypeDef* usbdHandle, USBD_SetupReqTypedef* req) {
  usbdStallEP (usbdHandle, 0x80);
  usbdStallEP (usbdHandle, 0);
  }
//}}}
//{{{
...
USBD_StatusTypeDef USBD_CtlSendData (USBD_HandleTypeDef* usbdHandle, uint8_t* buf, uint16_t len) {
  ...
  // Start the transfer
  usbdTransmit (usbdHandle, 0x00, buf, len);
  return USBD_OK;
  }
//}}}
//{{{
...
USBD_StatusTypeDef USBD_CtlPrepareRx (USBD_HandleTypeDef* usbdHandle, uint8_t* buf, uint16_t len) {
  ...
  // Start the transfer
  usbdPrepareReceive (usbdHandle, 0, buf, len);
  return USBD_OK;
  }
//}}}
//{{{
...
USBD_StatusTypeDef USBD_CtlSendStatus (USBD_HandleTypeDef* usbdHandle) {
  ...
  /* Start the transfer */
  usbdTransmit (usbdHandle, 0x00, NULL, 0);
...
USBD_StatusTypeDef USBD_CtlReceiveStatus (USBD_HandleTypeDef* usbdHandle) {
  ...
  /* Start the transfer */
  usbdPrepareReceive (usbdHandle, 0, NULL, 0);
...
uint16_t USBD_GetRxCount (USBD_HandleTypeDef* usbdHandle, uint8_t ep_addr) {
  return usbdGetRxDataSize(usbdHandle, ep_addr);
  }
//}}}

//{{{
USBD_StatusTypeDef usbdSetupStage (USBD_HandleTypeDef* usbdHandle, uint8_t* psetup) {
  ...
  switch (usbdHandle->request.bmRequest & 0x1F) {
    ...
    default:
      usbdStallEP (usbdHandle , usbdHandle->request.bmRequest & 0x80);
      break;
    }
...
//}}}
//{{{
USBD_StatusTypeDef usbdDataOutStage (USBD_HandleTypeDef* usbdHandle, uint8_t endPointNum, uint8_t* data) {
  ...
  if (endPointNum == 0) {
    ...
    if (usbdHandle->ep0_state == USBD_EP0_DATA_OUT) {
      if (pep->rem_length > pep->maxpacket) {
        ...
        USBD_CtlContinueRx (usbdHandle, data, MIN(pep->rem_length ,pep->maxpacket));
        }
      else {
...
//}}}
//{{{
USBD_StatusTypeDef usbdDataInStage (USBD_HandleTypeDef* usbdHandle, uint8_t endPointNum, uint8_t* data) {
  ...
  if (endPointNum == 0) {
    ...
    if (usbdHandle->ep0_state == USBD_EP0_DATA_IN) {
      if (pep->rem_length > pep->maxpacket) {
        pep->rem_length -=  pep->maxpacket;
        USBD_CtlContinueSendData (usbdHandle, data, pep->rem_length);
...
//}}}
//{{{
USBD_StatusTypeDef usbdReset (USBD_HandleTypeDef* usbdHandle) {

  // Open EP0 OUT
  usbdOpenEP (usbdHandle, 0x00, USBD_EP_TYPE_CTRL, USB_MAX_EP0_SIZE);
  usbdHandle->ep_out[0].maxpacket = USB_MAX_EP0_SIZE;

  // Open EP0 IN
  usbdOpenEP (usbdHandle, 0x80, USBD_EP_TYPE_CTRL, USB_MAX_EP0_SIZE);
...
//}}}
//{{{
USBD_StatusTypeDef usbdSetSpeed (USBD_HandleTypeDef* usbdHandle, USBD_SpeedTypeDef speed) {
...
//}}}
//{{{
USBD_StatusTypeDef usbdSuspend (USBD_HandleTypeDef* usbdHandle) {
...
//}}}
//{{{
USBD_StatusTypeDef usbdResume (USBD_HandleTypeDef* usbdHandle) {
...
//}}}
//{{{
USBD_StatusTypeDef usbdSOF (USBD_HandleTypeDef* usbdHandle) {
...
//}}}
USBD_StatusTypeDef usbdIsoINIncomplete (USBD_HandleTypeDef* usbdHandle, uint8_t endPointNum) { return USBD_OK; }
USBD_StatusTypeDef usbdIsoOUTIncomplete (USBD_HandleTypeDef* usbdHandle, uint8_t endPointNum) { return USBD_OK; }
USBD_StatusTypeDef usbdDevConnected (USBD_HandleTypeDef* usbdHandle) { return USBD_OK; }
//{{{
USBD_StatusTypeDef usbdDevDisconnected (USBD_HandleTypeDef* usbdHandle) {
...
//{{{
USBD_StatusTypeDef USBD_Init (USBD_HandleTypeDef* usbdHandle, USBD_DescriptorsTypeDef* descriptors, uint8_t id) {
...
USBD_StatusTypeDef USBD_DeInit (USBD_HandleTypeDef* usbdHandle) {
  ...
  // Initialize low level driver
  usbdDeInit (usbdHandle);

  return USBD_OK;
  }
//}}}
//{{{
USBD_StatusTypeDef USBD_RegisterClass (USBD_HandleTypeDef* usbdHandle, USBD_ClassTypeDef* classHandlers) {

  USBD_StatusTypeDef status = USBD_OK;
  if (classHandlers) {
    // link the class to the USB Device handle
    usbdHandle->pClass = classHandlers;
    status = USBD_OK;
    }
  else
    status = USBD_FAIL;
