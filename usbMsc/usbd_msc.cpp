...
void mscBotSendCsw (USBD_HandleTypeDef* usbdHandle, uint8_t CSW_Status) {
  ...
  usbdTransmit (usbdHandle, MSC_EPIN_ADDR, (uint8_t*)&mscData->csw, USBD_BOT_CSW_LENGTH);

  // Prepare EP to Receive next Cmd
  usbdPrepareReceive (usbdHandle, MSC_EPOUT_ADDR, (uint8_t*)&mscData->cbw, USBD_BOT_CBW_LENGTH);
  }
//}}}
//{{{
...
void mscBotAbort (USBD_HandleTypeDef* usbdHandle) {
  ...
  if ((mscData->cbw.bmFlags == 0) &&
    ...
    usbdStallEP (usbdHandle, MSC_EPOUT_ADDR);

  usbdStallEP (usbdHandle, MSC_EPIN_ADDR);
...
int8_t scsiRead (USBD_HandleTypeDef* usbdHandle, uint8_t lun) {
  ...
  usbdTransmit (usbdHandle, MSC_EPIN_ADDR, mscData->bot_data, len);
...
int8_t scsiWrite (USBD_HandleTypeDef* usbdHandle, uint8_t lun) {
  ...
  if (mscData->scsi_blk_len == 0)
    ...
    usbdPrepareReceive (usbdHandle, MSC_EPOUT_ADDR, mscData->bot_data, MIN(mscData->scsi_blk_len, MSC_MEDIA_PACKET));
...
void mscBotCbwDecode (USBD_HandleTypeDef* usbdHandle) {
  ...
  if ((usbdGetRxDataSize (usbdHandle ,MSC_EPOUT_ADDR) != USBD_BOT_CBW_LENGTH) ||
...
uint8_t mscInit (USBD_HandleTypeDef* usbdHandle, uint8_t cfgidx) {
  ...
  if (usbdHandle->dev_speed == USBD_SPEED_HIGH) {
    usbdOpenEP (usbdHandle, MSC_EPOUT_ADDR, USBD_EP_TYPE_BULK, MSC_MAX_HS_PACKET);
    usbdOpenEP (usbdHandle, MSC_EPIN_ADDR, USBD_EP_TYPE_BULK, MSC_MAX_HS_PACKET);
    }
  else {
    usbdOpenEP (usbdHandle, MSC_EPOUT_ADDR, USBD_EP_TYPE_BULK, MSC_MAX_FS_PACKET);
    usbdOpenEP (usbdHandle, MSC_EPIN_ADDR, USBD_EP_TYPE_BULK, MSC_MAX_FS_PACKET);
    }
  ...
  if (usbdHandle->pClassData) {
    ...
    usbdFlushEP (usbdHandle, MSC_EPOUT_ADDR);
    usbdFlushEP (usbdHandle, MSC_EPIN_ADDR);
...
uint8_t mscDeInit (USBD_HandleTypeDef* usbdHandle, uint8_t cfgidx) {
  ...
  // Close MSC EPs
  usbdCloseEP (usbdHandle, MSC_EPOUT_ADDR);
  usbdCloseEP (usbdHandle, MSC_EPIN_ADDR);
    ...
    case USB_REQ_TYPE_STANDARD: // Interface & Endpoint request
      switch (req->bRequest) {
        ...
        case USB_REQ_CLEAR_FEATURE:
          ...
          usbdFlushEP (usbdHandle, (uint8_t)req->wIndex);

          // Reactivate the EP
          usbdCloseEP (usbdHandle , (uint8_t)req->wIndex);
...
void HAL_PCD_SetupStageCallback (PCD_HandleTypeDef* pcdHandle) {
  usbdSetupStage ((USBD_HandleTypeDef*)pcdHandle->pData, (uint8_t*)pcdHandle->Setup);
  }
//}}}
//{{{
void HAL_PCD_DataOutStageCallback (PCD_HandleTypeDef* pcdHandle, uint8_t endPointNum) {
  usbdDataOutStage ((USBD_HandleTypeDef*)pcdHandle->pData, endPointNum, pcdHandle->OUT_ep[endPointNum].xfer_buff);
  }
//}}}
//{{{
void HAL_PCD_DataInStageCallback (PCD_HandleTypeDef* pcdHandle, uint8_t endPointNum) {
  usbdDataInStage ((USBD_HandleTypeDef*)pcdHandle->pData, endPointNum, pcdHandle->IN_ep[endPointNum].xfer_buff);
  }
//}}}
//{{{
void HAL_PCD_SOFCallback (PCD_HandleTypeDef* pcdHandle) {
  usbdSOF((USBD_HandleTypeDef*)pcdHandle->pData);
  }
//}}}
//{{{
void HAL_PCD_ResetCallback (PCD_HandleTypeDef* pcdHandle) {
  ...
  // Reset Device
  usbdReset ((USBD_HandleTypeDef*)(pcdHandle->pData));
  usbdSetSpeed ((USBD_HandleTypeDef*)(pcdHandle->pData), speed);
  }
//}}}
//{{{
void HAL_PCD_SuspendCallback (PCD_HandleTypeDef* pcdHandle) {
  usbdSuspend ((USBD_HandleTypeDef*)pcdHandle->pData);
  }
//}}}
//{{{
void HAL_PCD_ResumeCallback (PCD_HandleTypeDef* pcdHandle) {
  usbdResume ((USBD_HandleTypeDef*)pcdHandle->pData);
  }
//}}}
//{{{
void HAL_PCD_ISOOUTIncompleteCallback (PCD_HandleTypeDef* pcdHandle, uint8_t endPointNum) {
  usbdIsoOUTIncomplete ((USBD_HandleTypeDef*)pcdHandle->pData, endPointNum);
  }
//}}}
//{{{
void HAL_PCD_ISOINIncompleteCallback (PCD_HandleTypeDef* pcdHandle, uint8_t endPointNum) {
  usbdIsoINIncomplete ((USBD_HandleTypeDef*)pcdHandle->pData, endPointNum);
  }
//}}}
//{{{
void HAL_PCD_ConnectCallback (PCD_HandleTypeDef* pcdHandle) {
  usbdDevConnected ((USBD_HandleTypeDef*)pcdHandle->pData);
  }
//}}}
//{{{
void HAL_PCD_DisconnectCallback (PCD_HandleTypeDef* pcdHandle) {
  usbdDevDisconnected ((USBD_HandleTypeDef*)pcdHandle->pData);
  }
//}}}
...
//{{{
uint8_t usbdIsStallEP (USBD_HandleTypeDef* usbdHandle, uint8_t endPointAddress) {

  auto pcdHandle = (PCD_HandleTypeDef*)usbdHandle->pData;
  if ((endPointAddress & 0x80) == 0x80)
    return pcdHandle->IN_ep[endPointAddress & 0xF].is_stall;
  else
    return pcdHandle->OUT_ep[endPointAddress & 0xF].is_stall;
  }
//}}}
//{{{
uint32_t usbdGetRxDataSize (USBD_HandleTypeDef* usbdHandle, uint8_t endPointAddress) {
  return HAL_PCD_EP_GetRxCount ((PCD_HandleTypeDef*)usbdHandle->pData, endPointAddress);
  }
//}}}
