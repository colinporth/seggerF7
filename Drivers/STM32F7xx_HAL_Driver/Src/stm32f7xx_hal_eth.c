...
static void ETH_FlushTransmitFIFO (ETH_HandleTypeDef *heth) {
  ...
  // Wait until the write operation will be taken into account: at least four TX_CLK/RX_CLK clock cycles */
  __IO uint32_t tmpreg = (heth->Instance)->DMAOMR;
  HAL_Delay(ETH_REG_WRITE_DELAY);
  (heth->Instance)->DMAOMR = tmpreg;
  }
//}}}
...
HAL_StatusTypeDef HAL_ETH_Init (ETH_HandleTypeDef *heth) {
  ...
  // PHY initialization and configuration, Put the PHY in reset mode
...
__weak void HAL_ETH_MspInit (ETH_HandleTypeDef *heth) { UNUSED(heth); }
__weak void HAL_ETH_MspDeInit (ETH_HandleTypeDef *heth) { UNUSED(heth); }
__weak void HAL_ETH_TxCpltCallback (ETH_HandleTypeDef *heth) { UNUSED(heth); }
__weak void HAL_ETH_RxCpltCallback (ETH_HandleTypeDef *heth) { UNUSED(heth); }
__weak void HAL_ETH_ErrorCallback (ETH_HandleTypeDef *heth) { UNUSED(heth); }
